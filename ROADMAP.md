...
- [ ] I1 · FIDELITY 2.6 requires 60 fps at all three sizes and one geometry does not meet it: 1280×900 in the conn with a body filling the plate measures **32 fps** at the grid density the operator asked for. Everything else is 55-60, and 390×844 and 844×390 hold 60 in every view including a 218px body at full density — so this is desktop-with-a-planet-filling-the-screen only, and the platform the charter leads with is fine. Four things were tried and measured rather than assumed: batching ~190 stroke calls into 2 changed nothing; culling the off-screen limb and terminator changed nothing; capping lines × length cut a 1050px body from 28 latitude circles to 9 and bought 2-4 fps, so it came out again; dropping ctx.save/restore from the punch bought 9. The profile at that geometry: bodies 20 fps of the deficit (graticule 9, everything else 11), star field 4, CRT composite 6. It is fill-rate bound on stroked pixel area, so the remaining options are all quality trades: fewer lines, thinner lines, or a lower device pixel ratio when a body dominates the frame. Pick one deliberately or amend 2.6 with a stated exception — do not leave the charter claiming something the measurement contradicts. · blocked-by:
