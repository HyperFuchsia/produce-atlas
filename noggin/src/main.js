...
(function (NG) {
  ...
  /* The lowest the orbit may go, or null when the floor is out of its reach
     entirely and there is nothing to hit. */
  App.prototype._lowestPitch = function () {
    const want = (this.renderer.floorY + LENS_CLEAR - this.target[1])
      / Math.max(this.camera.dist, 0.001);
    if (want >= 1 || want <= -1) return null;
    return Math.asin(want);
  };
  ...
  App.prototype._clampToFloor = function () {
    ...
    if (lowest === null || c.pitch >= lowest) return;
  ...
  App.prototype.noticeHands = function (text) {
    ...
    /* With the prototype armed he does not answer the question at all. He
       takes the box off you. One or the other — both at once is him
       apologising while he does it, which is a different man. */
    if (this._canSnatch()) { c.armed = false; return; }
  ...
  /* ---- he takes the box off you ----------------------------------------- */

  /* The prototype. Everything else here answers you inside the frame; this
     reaches out of it.
  ...
  /* How far in front of the lens the glass is. It sets two things at once and
     ...
     room, and — because his hand has to be at the box — how big his hand
     looks. Close, and the box is a neat little slab and his hand fills the
     entire screen like a special effect. Far, and the hand is his own hand at
     his own scale.

     Measured both: at 5.6 the composer is a 33 cm panel, which is about what
     a bar spanning most of the window ought to be in a room where his head
     takes up a third of it, and his hand comes out at 297 px against a 334 px
     head. A hand is about the size of a face. That is the one that looks like
     a person picking something up. */
  const GLASS = 5.6;
  const HAND_BEHIND = 0.5;    /* his hand is behind the glass, as it would be */
  ...
  /* Screen pixels to a world point at a given distance from the eye. The
     ...
     frame for the pointer ray, so this is the same arithmetic run forwards. */
  App.prototype._screenToWorld = function (out, px, py, dist) {
    const nx = (px / this.cssW) * 2 - 1;
    const ny = 1 - (py / this.cssH) * 2;
    const d = [
      this.rayF[0] + this.rayR[0] * nx + this.rayU[0] * ny,
      this.rayF[1] + this.rayR[1] * nx + this.rayU[1] * ny,
      this.rayF[2] + this.rayR[2] * nx + this.rayU[2] * ny
    ];
    M.norm3(d, d);
  ...
  App.prototype._toScreen = function (p) {
    ...
    const nx = (vp[0] * p[0] + vp[4] * p[1] + vp[8] * p[2] + vp[12]) / w;
    const ny = (vp[1] * p[0] + vp[5] * p[1] + vp[9] * p[2] + vp[13]) / w;
  ...
  /* Called by the box when the word lands. Returning true means the box is
     his now and it should stop counting down to its own erase. */
  App.prototype.startSnatch = function () {
    const s = this.snatch;
    /* He needs a hand, and he needs to be someone with hands. A pineapple
       cannot do this either. */
    if (!this._canSnatch()) return false;
    this._spawnHands(this.morph.form);
    if (!this.hands) return false;

    const el = $('composer');
    s.el = el;
    s.home = this._boxHome(el);
    s.phase = 'reach';
    s.t = 0;
    s.blend = 0;
  ...
  App.prototype.updateSnatch = function (dt) {
    ...
    /* If he stops being someone with hands halfway through, he cannot finish,
       and you get your box back rather than watching it hang in the air. */
    if (!this.face || !this.hands) { this._giveBackBox(true); return; }

    s.t += dt;
    /* The glass moves with the camera, so where the box is stuck is worked
       out fresh rather than remembered. */
    s.home = this._boxHome(s.el);
    this._screenToWorld(s.glass, s.home.cx, s.home.cy, GLASS);

    if (s.phase === 'reach') {
      s.blend = M.smoothstep(0, 1, Math.min(1, s.t / SNATCH.reach));
      M.copy3(s.pos, s.glass);
      if (s.t >= SNATCH.reach) {
        s.phase = 'grip'; s.t = 0;
        this.audio.click();
      ...
      /* He has hold of it and it will not come. The pull shows up as the box
         straining toward him rather than as it moving, which is the whole
         difference between peeling something off and picking something up. */
      s.blend = 1;
      const k = Math.min(1, s.t / SNATCH.grip);
      s.resist = Math.sin(k * Math.PI) * (0.55 + 0.45 * Math.sin(this.time * 34));
      M.copy3(s.pos, s.glass);
      if (s.t >= SNATCH.grip) {
        s.phase = 'peel'; s.t = 0;
        s.resist = 0;
      ...
      /* Free. It follows his hand into the room, which is what takes it from
         being a thing on your screen to being a thing in there with him. */
      s.blend = 1;
  ...
  /* The one line that actually moves it. Everything above decides where the
     box is in the room; this asks the camera where that is on the screen. */
  App.prototype._drawBox = function () {
    const s = this.snatch;
    if (!s.el) return;
    const at = this._toScreen(s.pos);
    if (!at) { s.el.style.opacity = '0'; return; }
    s.el.style.opacity = '';
    const d = Math.max(0.2, M.dist3(this.eye, s.pos));
    const scale = GLASS / d;
    /* Stuck: while he is pulling and it is not coming, it strains toward him
       instead of moving — taller by a hair, and shivering. */
    const r = s.resist;
    const stretch = r > 0.001
      ? ' scaleY(' + (1 + r * 0.10).toFixed(4) + ') translate(0,' + (-r * 3).toFixed(2) + 'px)'
      : '';
    s.el.style.transform =
      'translate(' + (at.x - s.home.cx).toFixed(2) + 'px,' + (at.y - s.home.cy).toFixed(2) + 'px)'
      + ' scale(' + scale.toFixed(4) + ')'
      + ' perspective(760px) rotateX(' + s.tilt.toFixed(2) + 'deg)'
      + ' rotateZ(' + s.spin.toFixed(2) + 'deg)'
      + stretch;
  };
  ...
  /* Where the hand has to be to have hold of the text box: at the box, on the
     far side of the glass from you, palm to the window. */
  App.prototype._snatchHandPose = function () {
    const s = this.snatch;
    const at = this._screenToWorld([0, 0, 0], s.home.cx, s.home.cy, GLASS + HAND_BEHIND);
    at[1] -= HAND_HOLD;
    /* Away from you is into the room, which is the direction everything about
       this happens in. */
    const fwd = M.norm3([0, 0, 0], M.sub3([0, 0, 0], this.target, this.eye));

    if (s.phase === 'grip') {
      /* Hauling on something that is not moving: the arm goes back, the box
         does not. */
      const k = Math.min(1, s.t / SNATCH.grip);
    ...
    return {
      ...
      pitch: Math.asin(M.clamp(d[1], -1, 1)),
  ...
  App.prototype.updateHands = function (dt) {
    ...
    for (let i = 0; i < this.hands.length; i++) {
      ...
      if (took && i === s.hand) {
        ...
        py += (took.y - py) * b;
        pz += (took.z - pz) * b;
        let dy = (took.yaw - yaw) % (Math.PI * 2);
        if (dy > Math.PI) dy -= Math.PI * 2;
        if (dy < -Math.PI) dy += Math.PI * 2;
        yaw += dy * b;
        pitch += (took.pitch - pitch) * b;
      }
  ...
  /* Drawing, with no clock in it. Kept apart from `frame` so a still can be
     taken of the scene exactly as it stands, without a step of time going in
     first and changing what is being looked at. */
