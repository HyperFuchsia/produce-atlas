...
(function (NG) {
  ...
  App.prototype._wallHandPose = function (h) {
    ...
    const local = M.transformPoint([0, 0, 0], this.invModel, world);

    /* Face the palm at the camera. The mesh's palm normal is -Z, and the
       local matrix applies Ry then Rx, so the two angles fall straight out. */
    const at = M.transformPoint([0, 0, 0], this.invModel, this.eye);
    const d = M.norm3([0, 0, 0], M.sub3([0, 0, 0], at, local));
