...
(function (NG) {
  ...
  function App() {
    ...
    this.caught = { phase: 'off', t: 0, done: false, palm: 0, look: 0,
      at: { spin: 0, tilt: 0 } };
    this.focusDir = [0, 0, 1];
  ...
  App.prototype._setTrimmings = function (entry) {
    ...
    const c = this.caught;
    c.phase = 'off'; c.done = false; c.palm = 0; c.look = 0;
  ...
  /* The palms face outward, away from him, which is not how anybody holds
     ...
     looks down at one hand, then the other, apologises, and turns them round.
     You never get to tell him. He gets there first.

     It fires once per face. A thing that keeps apologising is not embarrassed,
     it is a machine with a trigger. */
  const CAUGHT = { left: 0.85, right: 0.80, turn: 0.75, back: 0.70 };

  App.prototype.noticeHands = function (text) {
    const c = this.caught;
    if (c.done || c.phase !== 'off') return;
    if (!this.face || !this.hands || this.morph.t < 1) return;
    if (this.fourthWall.phase !== 'off') return;
    if (!/hand/i.test(String(text || ''))) return;
  ...
  App.prototype.updateCaught = function (dt) {
    ...
    if (c.phase === 'turn') {
      /* Both palms come round to face him. */
    ...
    if (c.t >= CAUGHT.back) { c.phase = 'off'; c.look = 0; }
  };
  ...
  App.prototype.updateHands = function (dt) {
    ...
    for (let i = 0; i < this.hands.length; i++) {
      ...
      /* Palms out to begin with, which is wrong and is meant to be. They come
         round to face him the moment somebody notices — see updateCaught. */
      const palmYaw = -0.62 + (1.30 - -0.62) * this.caught.palm;
      let yaw = h.side * (palmYaw + h.lift * 0.34) + sway * 0.35;
      ...
      const tilt0 = -0.30 + (-0.06 - -0.30) * this.caught.palm;
      let pitch = tilt0 + bob * 0.9 - h.lift * (0.45 - 0.30 * this.caught.palm);
      let px = x, py = y, pz = z;
