...
(function () {
  ...
  function layDown(p, i, n) {
    ...
    p.group.position.set(Math.sin(a) * rad, 0.008 - minY, Math.cos(a) * rad);
    p.restY = p.group.position.y;
    p.fitted = false;
    p.anim = null;
  }
  ...
  function stepAssembly(dt) {
    for (var i = 0; i < schedule.length; i++) {
      ...
      var e = Math.min(1, p.anim.t);
      var k = e < 0.5 ? 4 * e * e * e : 1 - Math.pow(-2 * e + 2, 3) / 2;
      p.group.position.lerpVectors(p.anim.p0, p.fitPos, k);
      p.group.position.y += Math.sin(Math.PI * k) * 0.26;
      p.group.quaternion.slerpQuaternions(p.anim.q0, p.fitQuat, k);
      if (e >= 1) {
  ...
  function frame() {
    ...
    if (phase === "ASSEMBLY") { stepAssembly(dt); bobNext(clock.elapsedTime); }
