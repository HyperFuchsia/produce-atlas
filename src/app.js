...
(function () {
  ...
  var master = null, comp = null, meter = null, METER = null, keepAlive = null;
  /* A phone speaker is a centimetre across and has nothing under about 500 Hz,
     so everything here keeps its weight in the octave above where it would
     naturally sit. What it does NOT do is get loud: the drop is weather and
     the room is a room. The reels are the only thing allowed to be loud, and
     they are only loud relative to all of that. */
  var PUNCH = 1.0;
  var LOOPS = [], PLAYED = {};
  function tally(k) { PLAYED[k] = (PLAYED[k] || 0) + 1; }

  /* iOS hands Web Audio to the ambient audio session, and the ambient session
     is the one the ringer switch silences — so a phone with the switch flicked
     plays the whole thing to nobody. Declaring playback moves it to the media
     session, which the switch does not touch; on anything older than iOS 17
     an <audio> element that is genuinely playing does the same job. */
  function silentWav(seconds) {
    var rate = 8000, n = Math.floor(rate * seconds);
    ...
    var b = new Uint8Array(buf), sBin = "";
    for (var i = 0; i < b.length; i++) sBin += String.fromCharCode(b[i]);
    return "data:audio/wav;base64," + btoa(sBin);
  }
  function claimPlayback() {
    ...
    keepAlive.loop = true;
    keepAlive.volume = 1;
    keepAlive.src = silentWav(1);
    /* in the document, not merely constructed: a detached element does not
       hold the audio session, which is the entire reason it exists */
    keepAlive.setAttribute("aria-hidden", "true");
    keepAlive.style.cssText = "position:fixed;width:0;height:0;opacity:0;pointer-events:none";
    document.body.appendChild(keepAlive);
    var pr = keepAlive.play();
    if (pr && pr.catch) pr.catch(function () {});
  }
  /* Safari suspends the context whenever the page goes to the background and
     does not always bring it back, so every touch is also a nudge. */
  function resumeAudio() {
    if (!AC) return;
    if (AC.state === "suspended") { var r = AC.resume(); if (r && r.catch) r.catch(function () {}); }
    if (keepAlive && keepAlive.paused) { var q = keepAlive.play(); if (q && q.catch) q.catch(function () {}); }
  }
  function initAudio() {
    if (AC) { resumeAudio(); return AC; }
    claimPlayback();
    ...
    bus = AC.createGain();
    bus.gain.value = 0;
    /* Below seventy hertz nothing reaches a phone at all, and on the way past
       it robs the compressor of the headroom the audible half needs. */
    var hp = AC.createBiquadFilter();
    hp.type = "highpass"; hp.frequency.value = 70; hp.Q.value = 0.7;
    comp = AC.createDynamicsCompressor();
    /* glue, not a wall. At five to one over a threshold that low the mix had
       no dynamics left at all — a detent and a reel stop came out the same
       size, which is exactly the thing a slot machine must not do. */
    comp.threshold.value = -14; comp.knee.value = 12; comp.ratio.value = 2.5;
    comp.attack.value = 0.006; comp.release.value = 0.25;
    master = AC.createGain(); master.gain.value = 1.6;
    /* The compressor takes four milliseconds to lean on a transient, which is
       long enough for a detent or a reel stop to walk straight through it and
       come out the far side of the makeup gain at nearly twice full scale —
       where the output simply clips it. This one is fast and steep and does
       nothing else. */
    var lim = AC.createDynamicsCompressor();
    /* -4 rather than -2: even at twenty to one this thing overshoots its own
       threshold by a couple of dB on the sharpest transients, and the two dB
       of ceiling that buys are cheaper than the crackle */
    lim.threshold.value = -4; lim.knee.value = 0; lim.ratio.value = 20;
    lim.attack.value = 0.0008; lim.release.value = 0.06;
    /* and a hair off the top for whatever the limiter still lets past */
    var trim = AC.createGain(); trim.gain.value = 0.9;
    /* the output is read back rather than trusted: a graph that is connected
       and a graph that is making a sound are not the same claim */
    meter = AC.createAnalyser(); meter.fftSize = 2048;
    METER = new Float32Array(meter.fftSize);
    bus.connect(hp); hp.connect(comp); comp.connect(master);
    master.connect(lim); lim.connect(trim);
    trim.connect(meter); meter.connect(AC.destination);
    /* A running source held against the meter's input at zero offset. Without
       it the analyser has nothing to render once the graph goes quiet, keeps
       whatever was in its window, and reports the loudest thing that ever
       happened as though it were still happening. */
    var hold = AC.createConstantSource();
    hold.offset.value = 0;
    hold.connect(meter);
    hold.start();
  ...
  function burst(o) {
    ...
    g.gain.exponentialRampToValueAtTime(Math.max(0.0002, (o.gain || 0.3) * PUNCH), t + (o.a || 0.002));
  ...
  function tone(o) {
    ...
    var g = AC.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.exponentialRampToValueAtTime(Math.max(0.0002, (o.gain || 0.2) * PUNCH), t + (o.a || 0.004));
    g.gain.exponentialRampToValueAtTime(0.0001, t + dur);
  ...
  function loopOn(L, level, cutoff) {
    ...
    L.gain.gain.setTargetAtTime(muted ? 0 : level * PUNCH, aNow(), 0.06);
    if (cutoff && L.filter) L.filter.frequency.setTargetAtTime(cutoff, aNow(), 0.08);
  }
  function loopHz(L, hz) {
    if (L.on && L.pitch) L.pitch(hz, aNow());
  }
  ...
  function oscLoop(specs) {
    return function () {
      ...
      return {
        ...
        gain: g, filter: f,
        /* the whole partial stack moves together, so a rotor can be driven
           straight from how fast the drum is actually turning */
        pitch: function (hz, t) {
          var base = specs[0][0];
          for (var i = 0; i < oscs.length; i++) {
            oscs[i].frequency.setTargetAtTime(hz * specs[i][0] / base, t, 0.05);
          }
        }
      };
    };
  ...
  var WHIRR = loop(noiseLoop("bandpass", 900, 1.4));      /* bearings and air */
  /* The drum itself. A sawtooth stack whose pitch is the reel's own speed —
     which is why it rises through the wind-up and sags away as it comes down,
     without anything having to tell it to. */
  var MOTOR = loop(oscLoop([[64, 0.34, "sawtooth"], [128, 0.13], [192, 0.06]]));
  var HUM = loop(oscLoop([[50, 0.5], [100, 0.30], [150, 0.16], [200, 0.10],
                          [300, 0.05], [400, 0.028]]));
  ...
  function sClick(level) {                        /* a toggle, a keyway */
    tally("click");
    burst({ f0: 3200, f1: 1400, q: 2.2, gain: 0.075 * (level || 1), d: 0.035 });
    tone({ f0: 1500, f1: 700, d: 0.03, gain: 0.022 * (level || 1), type: "square" });
  }
  function sClack(level) {                        /* a relay, a key cap */
    tally("clack");
    level = level || 1;
    burst({ f0: 1900, f1: 620, q: 1.4, gain: 0.10 * level, d: 0.07 });
    tone({ f0: 380, f1: 180, d: 0.09, gain: 0.055 * level, type: "triangle" });
  }
  function sThunk(level) {                        /* a part seating, a lever catch */
    tally("thunk");
    level = level || 1;
    burst({ f0: 1100, f1: 280, q: 0.9, gain: 0.065 * level, d: 0.16 });
    /* the fundamental is for headphones; the octave above it is the only part
       of a thunk a handset will ever put in the air */
    tone({ f0: 150, f1: 60, d: 0.26, gain: 0.075 * level, type: "triangle" });
    tone({ f0: 330, f1: 150, d: 0.16, gain: 0.050 * level, type: "sine" });
    tone({ f0: 620, f1: 300, d: 0.09, gain: 0.025 * level, type: "sine" });
  }
  function sScrape(level) {                       /* steel dragged off concrete */
    tally("scrape");
    burst({ f0: 700, f1: 2600, q: 0.6, gain: 0.045 * (level || 1), d: 0.32, a: 0.06 });
  }
  function sPaper(level) {                        /* the docket feeding out */
    tally("paper");
    burst({ f0: 2600, f1: 5200, q: 0.5, gain: 0.045 * (level || 1), d: 0.30, a: 0.05 });
  }
  function sStep(level) {
    tally("step");
    burst({ f0: 420, f1: 190, q: 0.8, gain: 0.030 * (level || 1), d: 0.10 });
    burst({ f0: 2400, f1: 1100, q: 1.0, gain: 0.012 * (level || 1), d: 0.05 });
  }
  /* One symbol going past the window. A pawl dropping off a tooth is a pitched
     tick, not a hiss — a very short noise transient with a high Q on it, and a
     body under it that decays in thirty milliseconds. The pitch wanders a few
     per cent each time, because three hundred identical ticks in a row is a
     buzzer and no reel has ever sounded like a buzzer.
     `heavy` runs 0 to 1 as the drum slows: the ticks get lower and rounder as
     the thing loses its momentum. */
  function sDetent(level, heavy) {
    tally("detent");
    level = level || 1;
    heavy = heavy === undefined ? 0.5 : heavy;
    var f = (1680 - 340 * heavy) * (0.965 + Math.random() * 0.07);
    burst({ f0: f * 2.1, f1: f * 0.95, q: 7.0, gain: 0.082 * level, d: 0.013 });
    tone({ f0: f, f1: f * 0.80, d: 0.026 + 0.016 * heavy,
           gain: 0.060 * level, type: "triangle" });
  }
  /* A drum coming to rest does not stop, it arrives and then settles: the hit,
     and forty milliseconds later the small one where it rocks back into the
     detent. That second tick is most of what makes it sound mechanical. */
  function sReelStop(level) {
    tally("reelstop");
    level = level || 1;
    burst({ f0: 1500, f1: 380, q: 1.3, gain: 0.155 * level, d: 0.11 });
    tone({ f0: 196, f1: 82, d: 0.20, gain: 0.115 * level, type: "triangle" });
    tone({ f0: 480, f1: 210, d: 0.11, gain: 0.075 * level, type: "sine" });
    setTimeout(function () {
      burst({ f0: 2100, f1: 1200, q: 5.0, gain: 0.038 * level, d: 0.016 });
      tone({ f0: 620, f1: 470, d: 0.045, gain: 0.028 * level, type: "triangle" });
    }, 44);
  }
  ...
  function sCoin(level, size) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      var g = AC.createGain();
      var amp = (0.054 * level * PUNCH) / (1 + i * 1.5);
      g.gain.setValueAtTime(0.0001, t);
  ...
  function sRefuse() {
    tally("refuse");
    burst({ f0: 520, f1: 260, q: 1.6, gain: 0.075, d: 0.11 });
    tone({ f0: 192, f1: 148, d: 0.16, gain: 0.05, type: "square" });
  }
  ...
  /* peak and rms of what is actually leaving the graph */
  function levels() {
    if (!meter) return { peak: 0, rms: 0 };
    meter.getFloatTimeDomainData(METER);
    var pk = 0, sum = 0;
    for (var i = 0; i < METER.length; i++) {
      var v = METER[i]; sum += v * v; v = Math.abs(v);
      if (v > pk) pk = v;
    }
    return { peak: +pk.toFixed(4), rms: +Math.sqrt(sum / METER.length).toFixed(4) };
  }
  function setMuted(m) {
    muted = m;
    if (!audioOn) return;
  ...
  function mintFace(cls) {
    ...
    /* The authority's name round the top of the rim, as a coin has it. Each
       glyph is rotated to its own angle and set at the radius, and that is all:
       the extra half turn that used to be in here put every letter on its head,
       and a sweep of two hundred and eighty degrees carried the legend right
       round the bottom to meet itself. */
    var name = "PUBLIC LUCK AUTHORITY";
    var sweep = Math.PI * 0.94;
    x.save(); x.translate(180, 180);
    x.font = "600 22px " + MONO;
    for (var i = 0; i < name.length; i++) {
      var a2 = -sweep / 2 + (i / (name.length - 1)) * sweep;
      x.save(); x.rotate(a2); x.translate(0, -128);
  ...
  CLASSES.forEach(function (cls) {
    ...
    /* The reverse takes the same texture, unflipped. There used to be a mirrored
       copy here on the theory that a cylinder's two caps are seen from opposite
       sides — they are, but three already accounts for it in the cap UVs, and
       the flip was putting the mirror in rather than taking it out. What made
       coins read ETULAV ON was the rim legend, which was being drawn backwards
       in the canvas and so came out backwards on both faces. */
    var rev = face;
    ...
    /* The same coin, for the ones tumbling past you in the dark. At nine tenths
       metalness a face is a mirror, and out in the fog at a hundred metres
       there is nothing for it to reflect: they came out as black discs with a
       lit rim, which reads as a rendering fault rather than as money. These
       are duller and carry their own struck relief as emission, so they are
       legible against an empty sky without any light having to reach them. */
    var heroFace = new T.MeshPhysicalMaterial({
      map: face, color: cls.tone, roughness: 0.44, metalness: 0.30,
      emissive: 0x9AA0A6, emissiveMap: face, emissiveIntensity: 0.34
    });
    var heroRev = new T.MeshPhysicalMaterial({
      map: rev, color: cls.tone, roughness: 0.44, metalness: 0.30,
      emissive: 0x9AA0A6, emissiveMap: rev, emissiveIntensity: 0.34
    });
    var heroEdge = new T.MeshPhysicalMaterial({
      color: cls.tone, roughness: 0.50, metalness: 0.42, flatShading: true,
      emissive: 0x7E848A, emissiveIntensity: 0.26
    });
    cls.heroMats = [heroEdge, heroFace, heroRev];
  });
  ...
  function endDrag(e) {
    ...
    if (phase === "INTRO") {
      ...
      var tNow = simT;
      if (tNow - lastTap < 0.45) { skipIntro(); lastTap = 0; return; }
      lastTap = tNow;
      return;
    }
  ...
  var FALL_LINES = [
    ...
    [17.70, "I need the third to match. Three alike is the only one that counts.", 0.9],
    [18.30, "Come on. Come on. Come on —", 0.9],
    [19.10, "Yes. Yes —", 0.4],
    [19.45, "There we go.", 1.4]
  ];
  /* the light goes before the floor arrives */
  var T_FADE = 19.35, FADE_LEN = 0.42;
  var fallCue = 0;
  ...
  /* ------------------------------------------------------------ the stake --
     Three coins come out of the cabinet on the way down, one of each class,
     and the apparatus will not turn its last reel until it has been paid.
     Catching them is the only thing there is to do during the drop, and
     putting them in is what buys the result he is about to not get.

     They are held in front of the camera rather than dropped somewhere in the
     world: a coin two centimetres across, tumbling past at speed, on a phone,
  ...
  (function () {
    for (var i = 0; i < CLASSES.length; i++) {
      var cls = CLASSES[i];
      var m = new T.Mesh(cls.geo, cls.heroMats);
      m.visible = false;
      m.frustumCulled = false;
      FALLFX.add(m);
      HERO.push({
        mesh: m, cls: cls, idx: i, state: 0, t: 0, scale: 3.2,
        /* Stacked, not spread. Half the frame is only fifteen centimetres wide
           at this distance on a phone held upright but a good deal taller, so
           the room to separate three coins is vertical. They are spaced wider
           than the largest of them is across, which is what stops the biggest
           one sitting on top of the smallest. */
        lane: -0.085 + (i - 1) * 0.016, rise: (i - 1) * 0.125,
        off: new T.Vector3(0, 0, -1.15),
        spin: new T.Vector3(1.9 + i * 0.4, 2.6 - i * 0.3, 1.3 + i * 0.5)
      });
    }
  })();
  ...
  function fallHud() {
    ...
    hudRight.textContent = "COINS " + caught + " / 3";
  }
  /* the scripted line always wins: these are asides, not the script */
  function sayFree(text, hold) {
    if (sayUntil > simT) return;
    say(text, hold);
  }
  function catchHero(h) {
    if (h.state !== 1) return;
    h.state = 2; h.t = 0;
    caught++;
    sCoin(1.0, h.cls.r * 60);
    setTimeout(function () { sClick(0.7); }, 60);
  ...
  /* Everything here is smoothed in the eye's own frame and only then put into
     ...
     eight metres a second, and a lerp chasing a target that fast settles at a
     fixed distance behind it — the coins hung nearly two metres above the top
     of the frame and never came down. */
  function stepStake(dt, t) {
    if (!FALLFX.visible) return;
    camera.updateMatrixWorld();
    _cx.setFromMatrixColumn(camera.matrixWorld, 0);
    _cy.setFromMatrixColumn(camera.matrixWorld, 1);
    _cz.setFromMatrixColumn(camera.matrixWorld, 2);   /* points behind the eye */
    /* the slot, on the front of a cabinet that is tumbling as it falls */
    _slot.set(0, H * 0.74, D * 0.52);
    machine.localToWorld(_slot);
    for (var i = 0; i < HERO.length; i++) {
      var h = HERO[i], m = h.mesh, want = 3.2, rate = 6;
      if (h.state === 0) {
        if (t < HERO_AT[i]) continue;
        h.state = 1; h.t = 0;
        m.visible = true;
        /* it arrives from below, the way everything does when you are the
           thing that is falling fastest */
        h.off.set(h.lane, h.rise - 0.58, -1.15);
        h.scale = 3.2;
        sCoin(0.35, h.cls.r * 60);
        fallHud();
      }
      if (h.state === 4) continue;
      h.t += dt;
      if (h.state === 1) {
        var a = h.t * 0.85 + i * 2.1;
        _ct.set(h.lane + Math.sin(a) * 0.034,
                h.rise + Math.cos(a * 0.8) * 0.028,
                -1.15);
        want = 3.2; rate = 4.5;
      } else if (h.state === 2) {
        /* Pocketed: a row along the TOP of the frame. The bottom is where the
        ...
        _ct.copy(_slot).sub(camera.position);
        _ct.set(_ct.dot(_cx), _ct.dot(_cy), _ct.dot(_cz));
        want = 3.4; rate = 6;
        if (h.off.distanceTo(_ct) < 0.16) {
          h.state = 4; m.visible = false;
          credits++;
          sCoin(0.85, h.cls.r * 60);
      ...
      h.off.lerp(_ct, Math.min(1, rate * dt));
      m.position.copy(camera.position)
                .addScaledVector(_cx, h.off.x)
                .addScaledVector(_cy, h.off.y)
                .addScaledVector(_cz, h.off.z);
      h.scale += (want - h.scale) * Math.min(1, 7 * dt);
      m.scale.setScalar(h.scale);
      m.rotation.x += h.spin.x * dt;
  ...
  function pickHero(e) {
    ...
    ray.setFromCamera(ndc, camera);
    var loose = [];
    for (var i = 0; i < HERO.length; i++) if (HERO[i].state === 1) loose.push(HERO[i].mesh);
    if (!loose.length) return null;
    var hits = ray.intersectObjects(loose, false);
    if (!hits.length) return null;
    for (var j = 0; j < HERO.length; j++) if (HERO[j].mesh === hits[0].object) return HERO[j];
    return null;
  }
  function tappedMachine(e) {
    var r = canvas.getBoundingClientRect();
    ndc.x = ((e.clientX - r.left) / r.width) * 2 - 1;
    ndc.y = -((e.clientY - r.top) / r.height) * 2 + 1;
    ray.setFromCamera(ndc, camera);
  ...
  /* Coming round, not a fade-in: two blinks that half open and shut again
     before the eyes stay open. */
  var WAKE_OPACITY = [
    [0.00, 1.00], [2.40, 1.00], [2.78, 0.45], [3.30, 1.00],
    [3.95, 1.00], [4.50, 0.10], [5.05, 0.62], [6.30, 0.00]
  ];
  function wakeOpacity(t) {
    var k = WAKE_OPACITY;
  ...
  /* He never finds out. The display died with the machine, and the reel it
     died on is in a heap on the floor. */
  var WAKE_LINES = [
    [4.60,  "...how long was I out.", 2.2],
    [7.20,  "It was there. It was right there.", 2.4],
    [10.00, "I had it. I know I had it.", 2.4],
    [12.80, "Nothing on it now. The display's dead.", 2.6],
    [15.80, "I'll never know, will I.", 2.6],
    [19.20, "Why did I let him talk me into pulling that lever.", 3.4],
    [23.20, "It isn't permitted to be right about anybody. It says so on the front.", 4.0]
  ];
  function stepWake(dt) {
    ...
    if (was < 0.7 && wakeT >= 0.7) {
      unsilence(0.85);
      loopOn(RING, 0.014);
      loopOn(ROOM, 0.006, 180);
    }
    /* and then the room, as the ears let go of it */
    if (wakeT > 1.2) {
      var k = Math.min(1, (wakeT - 1.2) / 9.0);
      loopOn(RING, 0.014 * (1 - k));
      loopOn(ROOM, 0.006 + 0.014 * k, 180 + 260 * k);
  ...
  function startIntro() {
    ...
    reels.forEach(function (r, i) {
      ...
      r.aPos = undefined; r.lastDet = undefined;
    });
  ...
  function stepIntroReels(dt, t) {
    for (var i = 0; i < 3; i++) {
      ...
      if (i < 2) {
        ...
        if (e >= 1) { r.pos = r.introTarget; r.introDone = true; sReelStop(1.0); }
      } else {
        /* The stake is what releases it. Put the three coins in early and the
           last reel crawls the whole rest of the way down, which is a long
           time to watch one symbol approach another; leave it and the
           Authority starts it anyway at the scheduled hour, on account. */
        if (!r.crawl && t < T_CRAWL && credits < 3) { r.pos += 11.5 * dt; continue; }
        if (!r.crawl) {
          ...
          r.crawl = { from: r.pos, to: nextMatchAt(r.pos + 2.1) - 0.34, t: 0,
                      dur: Math.max(2.2, T_IMPACT - t) };
        }
        r.crawl.t += dt / r.crawl.dur;
  ...
  /* Above about seventeen symbols a second the ear stops resolving separate
     ticks and starts hearing a pitch, so that is where the drum hands over
     from detents to the rotor. Coming back down through it the ticks separate
     out again, one at a time, getting lower and heavier as they go — which is
     the entire sound of a slot machine slowing down, and the reason the third
     reel is worth waiting for. */
  var BLUR = 17;
  function reelSound(dt, level) {
    if (dt <= 0) return;
    var fastest = 0, anyMoving = false;
    for (var i = 0; i < 3; i++) {
      var r = reels[i];
      var prev = r.aPos === undefined ? r.pos : r.aPos;
      r.aPos = r.pos;
      var spd = Math.abs(r.pos - prev) / dt;
      /* a reel that has been re-seeded rather than turned is not moving */
      if (spd > 130) spd = 0;
      r.spd = spd;
      if (spd > 0.15) { anyMoving = true; if (spd > fastest) fastest = spd; }
      var f = Math.floor(r.pos);
      if (r.lastDet === undefined) { r.lastDet = f; continue; }
      if (f === r.lastDet) continue;
      r.lastDet = f;
      if (spd > BLUR || spd < 0.15) continue;       /* blurred, or not turning */
      var heavy = 1 - spd / BLUR;
      sDetent(level * (0.45 + 0.75 * heavy), heavy);
    }
    if (!anyMoving || cutAudio) {
      loopOff(MOTOR, 0.14); loopOff(WHIRR, 0.20);
      return;
    }
    /* six hertz per symbol per second: eleven and a half symbols a second — a
       free-running drum — sits at seventy, which is about where a real one
  ...
  function stepReels(dt) {
    /* first, because the drop drives the same drums from somewhere else and
       both of them have already moved by the time we get here */
    reelSound(dt, 1);
  ...
  load.innerHTML = "QA-77 · TAP TO BEGIN"
    + "<span class=\"sub\">WITH SOUND &middot; TURN IT UP</span>";
  ...
  window.QA77 = {
    ...
    /* how far short the third reel actually stopped. Nobody in the game ever
       sees this, which is rather the point of it. */
    unobserved: function () {
      var d = ((MATCH - reels[2].pos) % NSYM + NSYM) % NSYM;
    ...
    coinScreen: function () {
      ...
      camera.updateMatrixWorld();
      var right = new T.Vector3().setFromMatrixColumn(camera.matrixWorld, 0);
      return HERO.map(function (h) {
        var c = h.mesh.position.clone();
        var v = c.clone().project(camera);
        /* the same point pushed out by one coin radius, so a test can ask
           whether two of them actually overlap rather than guessing a number */
        var e = c.clone().addScaledVector(right, h.cls.r * h.scale).project(camera);
        return { cls: h.cls.name, state: h.state,
                 x: +v.x.toFixed(3), y: +v.y.toFixed(3), z: +v.z.toFixed(3),
                 r: +Math.abs(e.x - v.x).toFixed(3) };
      });
    },
    ...
    /* one sound at a time, in silence, so the balance between them can be
       measured rather than guessed at from a mix */
    sfx: {
      detent: sDetent, reelStop: sReelStop, click: sClick, clack: sClack,
      thunk: sThunk, coin: sCoin, refuse: sRefuse, paper: sPaper,
      step: sStep, scrape: sScrape
    },
    /* the sound cannot be listened to from a test, so it is counted instead */
    audio: function () {
      return {
        on: audioOn, state: AC ? AC.state : null, muted: muted,
        time: AC ? +AC.currentTime.toFixed(3) : 0,
