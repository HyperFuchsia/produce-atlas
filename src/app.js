...
(function () {
  ...
  /* A phone speaker is a centimetre across, has nothing under about 500 Hz,
     and is being held at arm's length in a room. Everything here is scaled
     for that and then let through a compressor doing real work, because on
     that speaker an honest dynamic range is the same thing as inaudible. */
  var PUNCH = 1.7;
  var LOOPS = [], PLAYED = {};
  ...
  function initAudio() {
    ...
    comp.threshold.value = -22; comp.knee.value = 8; comp.ratio.value = 5;
  ...
  function sClack(level) {                        /* a relay, a key cap */
    tally("clack");
    level = level || 1;
    burst({ f0: 1900, f1: 620, q: 1.4, gain: 0.24 * level, d: 0.07 });
    tone({ f0: 380, f1: 180, d: 0.09, gain: 0.16 * level, type: "triangle" });
  }
  function sThunk(level) {                        /* a part seating, a lever catch */
    tally("thunk");
    level = level || 1;
    burst({ f0: 1100, f1: 280, q: 0.9, gain: 0.26 * level, d: 0.16 });
    /* the fundamental is for headphones; the octave above it is the only part
       of a thunk a handset will ever put in the air */
    tone({ f0: 150, f1: 60, d: 0.26, gain: 0.34 * level, type: "triangle" });
    tone({ f0: 330, f1: 150, d: 0.16, gain: 0.22 * level, type: "sine" });
    tone({ f0: 620, f1: 300, d: 0.09, gain: 0.10 * level, type: "sine" });
  }
  ...
  function sStep(level) {
    tally("step");
    burst({ f0: 420, f1: 190, q: 0.8, gain: 0.085 * (level || 1), d: 0.10 });
  ...
  function sReelStop(level) {
    tally("reelstop");
    level = level || 1;
    burst({ f0: 1400, f1: 340, q: 1.1, gain: 0.32 * level, d: 0.13 });
    tone({ f0: 155, f1: 62, d: 0.22, gain: 0.26 * level, type: "triangle" });
    tone({ f0: 470, f1: 190, d: 0.13, gain: 0.20 * level, type: "sine" });
  }
  ...
  function sCoin(level, size) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      var g = AC.createGain();
      var amp = (0.16 * level * PUNCH) / (1 + i * 1.5);
      g.gain.setValueAtTime(0.0001, t);
  ...
  function sRefuse() {
    tally("refuse");
    burst({ f0: 520, f1: 260, q: 1.6, gain: 0.18, d: 0.11 });
    tone({ f0: 192, f1: 148, d: 0.16, gain: 0.13, type: "square" });
  }
