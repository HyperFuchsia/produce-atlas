...
(function () {
  ...
  function dolly(scale) {
    autoFrame = false;                    /* the viewer has taken the wheel */
    var f = fitDistance();
  ...
  function endDrag(e) {
    ...
    if (dragging && travel < 6 && e) {
      if (phase === "INTRO" || phase === "BREAK") {
        skipIntro(); lastTap = 0; dragging = false;
        canvas.classList.remove("dragging"); return;
      }
      if (phase === "ASSEMBLY") {
        var part = pickPart(e);
        if (part) { tryFitPart(part); lastTap = 0; dragging = false;
                    canvas.classList.remove("dragging"); return; }
      }
      var hit = pick(e);
  ...
  function stepCommission(dt) {
    ...
    orbit.pitch += (PITCH_RUN - orbit.pitch) * Math.min(1, 1.4 * dt);
    if (commissionT > 2.6) {
      phase = "OPERATION";
      power = 1; applyPower();
  ...
  var FALL_H = 7.4, MATCH = 3;                   /* MELON, three alike */
  var T_STOP = [2.30, 3.55], T_CRAWL = 4.05, T_IMPACT = 6.90;
  var introT = 0, broke = false;
  ...
  function startIntro() {
    planScatter();
    phase = "INTRO"; introT = 0; broke = false;
    power = 1; applyPower();
  ...
  function stepIntro(dt) {
    ...
    /* falling, and straightening as it falls */
    machine.position.y = FALL_H * (1 - u * u);
    machine.rotation.y = 0.62 * (1 - u);
    machine.rotation.z = 0.20 * (1 - u) * Math.sin(introT * 1.9);
    machine.rotation.x = 0.14 * (1 - u) * Math.cos(introT * 1.5);
    /* the shadow tightening under it is most of what says how close it is */
    contact.material.opacity = 0.10 + 0.80 * u * u;
    var sp = 2.6 - 1.6 * u * u;
    contact.scale.set(sp, sp, sp);

    orbit.target.set(0, machine.position.y + H * 0.52, 0);
    orbit.yaw = -0.66 + 0.34 * u;
    orbit.pitch = -0.05 + 0.42 * u * u;
    orbit.dist = 4.0 - 0.8 * u;
  ...
  function skipIntro() {
    ...
    scatterNow();
    phase = "ASSEMBLY";
    autoFrame = true; orbit.pitch = PITCH_BUILD; retarget();
  ...
  function frame() {
    ...
    if (phase === "INTRO") stepIntro(dt);
      ...
      stepAssembly(dt);
      orbit.pitch += (PITCH_BUILD - orbit.pitch) * Math.min(1, 1.6 * dt);
    }
